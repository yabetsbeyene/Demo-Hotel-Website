...
export async function getDashboardData() {
  const [roomTypes, arrivals, departures, revenue, activity, recentBookings] = await Promise.all([
    ...
    query<any>(`SELECT b.id, b.confirmation_code, b.guest_full_name, b.guest_email, b.check_in, b.check_out, b.created_at, rt.name AS room_type_name FROM bookings b JOIN room_types rt ON rt.id = b.room_type_id WHERE b.status = 'pending' ORDER BY b.created_at DESC LIMIT 5`)
  ]);
  ...
  return { roomTypes: roomTypes.rows, arrivals: arrivals.rows, departures: Number(departures.rows[0]?.count ?? 0), revenue: revenue.rows[0]?.total ?? 0, activity: activity.rows, recentBookings: recentBookings.rows, occupancy: totalRooms ? Math.round((occupiedRooms / totalRooms) * 100) : 0, totalRooms, occupiedRooms };
}
